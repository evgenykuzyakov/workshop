...
Вы этом семинаре вы научитесь основам смарт-контрактов на языке Rust для NEAR Protocol.

## Подготовка

### Установка

Описанный процесс походит для систем Unix, например Linux или Mac OS. Процесс установки для Windows может отличаться.
...
Web-wallet создаст новый аккаунт на тестовый сети testnet и добавить 200 тестовых NEAR токенов на этот аккаунт.
Запомните свой полный account ID, например `alice.testnet`

В качестве `метода восстановления/Security Method` для этого семинара я рекомендую использовать `Мнемоническую фразу/Recovery Phrase` или `Электронная почта/Email Recovery`.
...
Как только вы разрешите доступ в брауере, командая строка должна успешно завершиться.

Вы должные увидеть что то подобное в терминале:
```
...
Чтобы упростить этот семинар, нужно сохранить ваше имя использователя в переменную `ACCOUNT_ID` в bash.
...
Это значит `test_rect_complex` не работает. И assert в файле `src/rect.rs` строка `90` не верный.
Откройте этот файл и пропробуйте разобраться что пошло не так.
После того как вы найдете ошибку и исправите ее, то перезапустите тесты.

Повторяйте этот процесс пока не исправите все ошибки в коде (не в тестах).

Как только все ошибки испавлены и тесты работают, можно переходить к часть 2.

## Часть 2 - Порисуем

В этой часте вам нужно будет изменить новый метод в смарт контракте, чтобы нарисовать что-то посложнее.
...
Но если вы будете использовать слишком много точек, то это может отразиться на производительности.
...
Результат должен быть в файле `res/berry_bot.wasm`

### Добавляем контракт на аккаунт

Аккаунты в NEAR Protocol также могут содержать смарт контракт.
Смарт контракт имеет полный доступ к аккаунту на котором он разположен.
...
Вы должны увидеть чтото подобное
```
...
Вы должны увидеть холст 50x50. Он может быть немного грязный потому что на нем рисуют все (одновременно и по очереди).

Оставьте эту вкладку в браузере открытой.
...
А также ваш аккаунт должен появится в списке спава от холста.
Если вы наведете курсор над своим аккаунтом в списке, то это должно подсветить ваш рисунок и ваши точки.
...
Помните, если вы изменили код, то его нужно перекомпилировать и добавить заного на аккаунт.
